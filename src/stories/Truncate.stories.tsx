...
const data2 = ['San Jose', 'Single bed', 'Luxury', 'XXX', 'Secret-Door', 'Fire-Place']
...
const Tag = forwardRef<TagRef, TagProps>(({children, ...props}, ref) => (
        <div ref={ref} style={{padding:'3px 5px', background: '#bababa', flexShrink: 0, marginRight:5}} {...props}>
            {children}
        </div>
    )
)
type EllipsisProps = {moreCount: number; moreChildren:[React.ReactNode]}
const Ellipsis = forwardRef<TagRef, EllipsisProps>(({moreCount, moreChildren, ...props}, ref) => {
    return(
        <div ref={ref} {...props} style={{flexShrink:0}}>+{moreCount} tags</div>
    )
})
        ...
        <div>
            ...
            <div style={{width:350, background: 'blue', padding:10}}>
                <Truncate isTruncating={isTruncating}>
                    <div style={{display: 'flex', flexWrap: 'wrap', alignItems:'center', background: 'lightcyan'}}>
