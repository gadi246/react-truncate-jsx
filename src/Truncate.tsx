...
function Truncate({children, isTruncating = true}:TruncateProps){
        ...
        return  new IntersectionObserver(
            entries => {
                entries.forEach((entry:IntersectionObserverEntry) => {
                    if(entry.isIntersecting && entry.intersectionRatio < 1 && entry.target instanceof HTMLElement){
    ...
    const newWrapper =  React.cloneElement(wrapper, {
        style:{...wrapper.props.style, ...(isTruncating ? {overflow: 'hidden', flexWrap: 'nowrap'} : {})},
        ref
    });

    const context: TruncateContextInterface = {observer, truncateIndex, isTruncating}
    return(
